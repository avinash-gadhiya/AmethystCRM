...
export default function NavCollapse({ collapse, type }) {
  ...
  useEffect(() => {
    if (isChildActive || segments.includes(collapseIdLower)) {
      dispatch({ type: actionType.COLLAPSE_OPEN, menu: { id: collapse.id } });
    }
  }, [collapse.id, collapseIdLower, isChildActive, location.pathname, dispatch]);
